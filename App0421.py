...
class DacScanThread(QThread):
    ...
    def run(self):
        ...
        while code < 256:
            ...
            time.sleep(0.1)
            amp = self.brd_config.cots_adc_data(self.N)
            self.result.append(amp)
            self.mutex.unlock()
            self.interact.emit(self.result)
            QThread.msleep(100)
#---------------MainWindow---------------#    
...
#        layout5.addWidget(self.btn_CodeScanStop)            
    ...
    def COTS_ADC(self):
        ...
        layout.addWidget(self.btn_dacscan_start,2,0)
        layout.addWidget(self.btn_dacscan_stop,2,1)
...
#
    ...
    def btn_dacscan_start_clicked(self):
        ...
        N = int(self.ledit_cots_avr_points.text(),0)
#        self.DacScanThread_inst = DacScanThread(reg,N)
...
#        self.DacScanThread_inst.start()        
    ...
    def Update_DACSCAN(self,result1):
        if self.DacScanThread_inst.mutex.tryLock():
            ...
            self.dacscanplot.setData(result)
    #------------------------------------------------#
    #---------FE control--------#
