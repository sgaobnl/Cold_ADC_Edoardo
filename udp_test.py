...
class UDP_frame:
    ...
    def __init__(self,udp_frame_ptr,raw_data):
        self.udp_frame_length = 3836 #in words
    ...
    def write_reg(self, reg, data, femb_addr = None):
        ...
        if(Value == None):
            ...
            if femb_addr == None:#wib mode
                ...
                self.sock_write_reg_wib.sendto(WRITE_MESSAGE,(self.UDP_IP, self.UDP_PORT_WREG ))
                #self.sock_write.sendto(WRITE_MESSAGE,(self.UDP_IP, self.UDP_PORT_WREG ))      #WIB   WREG UDP port 32000
            elif femb_addr == 0:
                self.sock_write_reg_femb0.sendto(WRITE_MESSAGE,(self.UDP_IP, self.UDPFEMB0_PORT_WREG  ))
                #self.sock_write.sendto(WRITE_MESSAGE,(self.UDP_IP, self.UDPFEMB0_PORT_WREG  ))#FEMB0 WREG UDP port 32016
            elif femb_addr == 1:
                self.sock_write_reg_femb1.sendto(WRITE_MESSAGE,(self.UDP_IP, self.UDPFEMB1_PORT_WREG  ))
                #self.sock_write.sendto(WRITE_MESSAGE,(self.UDP_IP, self.UDPFEMB1_PORT_WREG  ))#FEMB1 WREG UDP port 32032
            elif femb_addr == 2:
                self.sock_write_reg_femb2.sendto(WRITE_MESSAGE,(self.UDP_IP, self.UDPFEMB2_PORT_WREG  ))
                #self.sock_write.sendto(WRITE_MESSAGE,(self.UDP_IP, self.UDPFEMB2_PORT_WREG  ))#FEMB2 WREG UDP port 32048
            elif femb_addr == 3:
                self.sock_write_reg_femb3.sendto(WRITE_MESSAGE,(self.UDP_IP, self.UDPFEMB3_PORT_WREG  ))
                #self.sock_write.sendto(WRITE_MESSAGE,(self.UDP_IP, self.UDPFEMB3_PORT_WREG  ))#FEMB3 WREG UDP port 32064
            # Close the socket
            #sock_write.close()
    ...
    def read_reg(self, reg, femb_addr = None ):
        ...
        if (regVal < 0) or (regVal > self.MAX_REG_NUM):
           ...
           return None
            ...
            data = []
            if femb_addr == None:
                self.sock_read_reg_wib.sendto(READ_MESSAGE,(self.UDP_IP,self.UDP_PORT_RREG))
                try:
                    data = self.sock_readresp_wib.recv(4*1024)
                except socket.timeout:
                    self.udp_timeout_cnt = self.udp_timeout_cnt  + 1
                    return -1                        
                #self.sock_readresp.bind(('', self.UDP_PORT_RREGRESP ))      #WIB Response UDP port 32002
                #self.sock_read.sendto(READ_MESSAGE,(self.UDP_IP,self.UDP_PORT_RREG))#Read request UDP port 32001
            elif femb_addr == 0:
                self.sock_read_reg_femb0.sendto(READ_MESSAGE,(self.UDP_IP,self.UDPFEMB0_PORT_RREG))
                try:
                    data = self.sock_readresp_femb0.recv(4*1024)
                except socket.timeout:
                    self.udp_timeout_cnt = self.udp_timeout_cnt  + 1
                    return -1                   
                #self.sock_readresp.bind(('', self.UDPFEMB0_PORT_RREGRESP )) #FEMB0 Response UDP port 32018
                #self.sock_read.sendto(READ_MESSAGE,(self.UDP_IP,self.UDPFEMB0_PORT_RREG)) #FEMB0 Read request UDP port 32017
            elif femb_addr == 1:
                self.sock_read_reg_femb1.sendto(READ_MESSAGE,(self.UDP_IP,self.UDPFEMB1_PORT_RREG))
                try:
                    data = self.sock_readresp_femb1.recv(4*1024)
                except socket.timeout:
                    self.udp_timeout_cnt = self.udp_timeout_cnt  + 1
                    return -1                   
                #self.sock_readresp.bind(('', self.UDPFEMB1_PORT_RREGRESP )) #FEMB1 Response UDP port 32034
                #self.sock_read.sendto(READ_MESSAGE,(self.UDP_IP,self.UDPFEMB1_PORT_RREG)) #FEMB1 Read request UDP port 32033
            elif femb_addr == 2:
                self.sock_read_reg_femb2.sendto(READ_MESSAGE,(self.UDP_IP,self.UDPFEMB2_PORT_RREG))
                try:
                    data = self.sock_readresp_femb2.recv(4*1024)
                except socket.timeout:
                    self.udp_timeout_cnt = self.udp_timeout_cnt  + 1
                    return -1                
                #self.sock_readresp.bind(('', self.UDPFEMB2_PORT_RREGRESP )) #FEMB2 Response UDP port 32050
                #self.sock_read.sendto(READ_MESSAGE,(self.UDP_IP,self.UDPFEMB2_PORT_RREG)) #FEMB2 Read request UDP port 32049
            elif femb_addr == 3:
                self.sock_read_reg_femb3.sendto(READ_MESSAGE,(self.UDP_IP,self.UDPFEMB3_PORT_RREG))
                try:
                    data = self.sock_readresp_femb3.recv(4*1024)
                except socket.timeout:
                    self.udp_timeout_cnt = self.udp_timeout_cnt  + 1
                    return -1                
                #self.sock_readresp.bind(('', self.UDPFEMB3_PORT_RREGRESP )) #FEMB3 Response UDP port 32066
                #self.sock_read.sendto(READ_MESSAGE,(self.UDP_IP,self.UDPFEMB3_PORT_RREG)) #FEMB3 Read request UDP port 32065
            #sock_read.close()
            #try to receive response packet from board, store in hex
#            try:
#                data = self.sock_readresp.recv(4*1024)
#            except socket.timeout:
#                self.udp_timeout_cnt = self.udp_timeout_cnt  + 1
...
#Get high-speed data
    def get_rawdata(self,PktNum,checkflg,Jumbo=None):
        ...
        for i in range(cycle):    
            data = None
            try:
                data = self.sock_hs_data.recv(recvbuf) 
            except socket.timeout:
...
# 建立先套接字解决采数时发送命令的卡顿问题
    def sockets_setup(self):
        self.sock_write_reg_wib = self.socket_gen("Write")
        #self.sock_write_reg_wib.bind((self.host, self.UDP_PORT_WREG))
        
        self.sock_read_reg_wib  = self.socket_gen("Write")
        #self.sock_read_reg_wib.bind((self.host, self.UDP_PORT_RREG))
        
        self.sock_readresp_wib  = self.socket_gen("Listen")
        self.sock_readresp_wib.bind((self.host, self.UDP_PORT_RREGRESP))
        
        self.sock_hs_data = self.socket_gen("Listen")
        self.sock_hs_data.bind((self.host,self.UDP_PORT_HSDATA))

        self.sock_write_reg_femb0 = self.socket_gen("Write")
        #self.sock_write_reg_femb0.bind((self.host, self.UDPFEMB0_PORT_WREG))
        
        self.sock_read_reg_femb0  = self.socket_gen("Write")
        #self.sock_read_reg_femb0.bind((self.host, self.UDPFEMB0_PORT_RREG))
        
        self.sock_readresp_femb0  = self.socket_gen("Listen")
        self.sock_readresp_femb0.bind((self.host, self.UDPFEMB0_PORT_RREGRESP))    

        self.sock_write_reg_femb1 = self.socket_gen("Write")
        #self.sock_write_reg_femb1.bind((self.host, self.UDPFEMB1_PORT_WREG))
        
        self.sock_read_reg_femb1  = self.socket_gen("Write")
        #self.sock_read_reg_femb1.bind((self.host, self.UDPFEMB1_PORT_RREG))
        
        self.sock_readresp_femb1  = self.socket_gen("Listen")
        self.sock_readresp_femb1.bind((self.host, self.UDPFEMB1_PORT_RREGRESP))  

        self.sock_write_reg_femb2 = self.socket_gen("Write")
        #self.sock_write_reg_femb2.bind((self.host, self.UDPFEMB2_PORT_WREG))
        
        self.sock_read_reg_femb2  = self.socket_gen("Write")
        #self.sock_read_reg_femb2.bind((self.host, self.UDPFEMB2_PORT_RREG))
        
        self.sock_readresp_femb2  = self.socket_gen("Listen")
        self.sock_readresp_femb2.bind((self.host, self.UDPFEMB2_PORT_RREGRESP))  

        self.sock_write_reg_femb3 = self.socket_gen("Write")
        #self.sock_write_reg_femb3.bind((self.host, self.UDPFEMB3_PORT_WREG))
        
        self.sock_read_reg_femb3  = self.socket_gen("Write")
        #self.sock_read_reg_femb3.bind((self.host, self.UDPFEMB3_PORT_RREG))
        
        self.sock_readresp_femb3  = self.socket_gen("Listen")
        self.sock_readresp_femb3.bind((self.host, self.UDPFEMB3_PORT_RREGRESP))           
    def sockets_kill(self):
        self.sock_write_reg_wib.close() 
        self.sock_read_reg_wib.close()
        self.sock_readresp_wib.close()   
        self.sock_hs_data.close()
        
        self.sock_write_reg_femb0.close()
        self.sock_read_reg_femb0.close()
        self.sock_readresp_femb0.close()
        
        self.sock_write_reg_femb1.close()
        self.sock_read_reg_femb1.close()
        self.sock_readresp_femb1.close()
        
        self.sock_write_reg_femb2.close()
        self.sock_read_reg_femb2.close()
        self.sock_readresp_femb2.close()
        
        self.sock_write_reg_femb3.close()
        self.sock_read_reg_femb3.close()
        self.sock_readresp_femb3.close()        
    #__INIT__#
    def __init__(self):
        self.bitop = Bit_Op()
