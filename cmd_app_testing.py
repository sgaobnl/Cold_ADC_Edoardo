...
"""
# This file can be used for performance testing (or characterization debugging) of ColdADC. 
# Complete setting of ColdADC, with calibration weights (fixed or manual calibration).
# Test options: external input (channels) or internal ADC (ADC test input), free or frozen SHA, with and without SDCs, crosstalk tests.
# Saves all binary data in specified folder.
...
while(woc_f==False):
    cq.init_chk()
    cq.ref_set(flg_bjt_r = flg_bjt_r, env=env)
    time.sleep(1)
    cq.all_ref_vmons( )
...
## Fixed Calibration Weights ##
##filename = "D:/Cold_ADC/python_script/cali_weights/BJT_all_2V5_2V1_warm.txt"
#filename = "D:/Cold_ADC/python_script/cali_weights/CMOS_all_2V5_2V1_warm_brd2.txt"
...
#            
#            rawdata = cq.get_adcdata_raw(PktNum=4000000 )
#            chns = cq.raw_conv(rawdata)
...
#
##### SDC Enable #####
#print ("Enabling Single-to-Differential Converters...")
#cq.Input_buffer_cfg(sdc = "On", db = "Bypass", sha = "Diff", curr_src = "BJT-sd")    
#
#
##### Freeze SHA  #####
#print ("Freeze SHA")
#cq.bc.adc_freeze_sha0("Freeze")
#cq.bc.adc_freeze_select_0(7)
#cq.bc.adc_freeze_sha1("Freeze")
#cq.bc.adc_freeze_select_1(7)


input("Waiting for Sine Wave Generator power-on. When ready, press Enter to continue...")



rawdir = rawdir + "ADC_TST_IN/"
...
#General Testing
#Ntot = 2**(14)
#Ntot = 131072 used for ADC only input with averaged data
Ntot = 2**(22)
#Ntot = 60000
chns = cq.get_adcdata(PktNum=Ntot )
fn = rawdir + "ADC_TST_IN" + "_test" + ".bin"
print (fn)
with open(fn, 'wb') as f:
    pickle.dump(chns, f)
...
##Linearity Sine Wave Testing
#Ntot = 2**(22)
#fin = 21 #kHz
#vpp = 1.3 #V
#offset = 1.20 #V, actual offset (double than what seen from generator)
#rawdata = cq.get_adcdata_raw(PktNum=Ntot )
#chns = raw_conv(rawdata)
#fn = rawdir + "lin" + "_%0.0fk"%fin + "_%0.1fv"%vpp + "_%0.2fv"%offset + ".bin"
#print (fn)
#with open(fn, 'wb') as f:
#    pickle.dump(chns, f)
...
#rawdata = cq.get_adcdata_raw(PktNum=Ntot )
#chns = raw_conv(rawdata)
#fn = rawdir + "ac_%d"%Ntot + "_%0.4f"%fin + ".bin"
#print (fn)
#with open(fn, 'wb') as f:
#    pickle.dump(chns, f)
